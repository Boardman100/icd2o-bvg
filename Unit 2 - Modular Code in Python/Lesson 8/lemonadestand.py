...
def num_customers(weather,luck,ads,price,numcups):
    if weather == "stormy":
        num_customers = math.floor(numcups*(luck/100+ads*0.1)*(1-price*0.01))
    if weather == "rainy":
        num_customers = math.floor(numcups*(luck/100+ads*0.1)*(1.2-price*0.01))
    if weather == "cool":
        num_customers = math.floor(numcups*(luck/100+ads*0.1)*(1.3-price*0.01))
    if weather == "warm":
        num_customers = math.floor(numcups*(luck/100+ads*0.1)*(1.6-price*0.01))
    if weather == "hot":
...
while day < 15:
    ...
    print(f"Today the weather is {weather}")
    pricepercup = int(input("How many cents would you like to charge per cup of lemonade? "))
    numcups = int(input(f"Cups now cost 6 cents to produce. How many cups would you like to make? "))
    numads = int(input(f"If ads cost 10 cents each. How many ads would you like to post? "))
    cupssold = num_customers(weather,luck,numads,pricepercup,numcups)
    assets = cupssold*pricepercup
    expenses = numcups*3+numads*15
    netprofit = assets-expenses
